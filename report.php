<?php
$file = file_get_contents('test/json.json');
$file_decode = json_decode($file,TRUE);
$subjects = array();
$score_info = array();
$test_array = array();
// array for duplicates elements
$ex = array();
...
foreach ($students as $key => $value) {
    foreach($value['exam_result'] as $row => $data){
        ...
        foreach($data as $info => $get) {
            ...
            // if(in_array($get['name'], $subjects)) {
            //     // $students[$get['name']] => $get['name'];
            //     // echo '123';
            //     array_push($ex, $get['name']);
            //     array_replace($ex,$subjects);
            // }else {
            //     array_push($subjects, $get['name']);
            // }
...
// var_dump(array_unique($subjects));
// var_dump($score_info);
// var_dump($test_array);
